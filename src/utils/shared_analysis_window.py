"""Fixed upper date boundary for analyses, independent of the data snapshot date.
...
def filter_analysis_window(
    ...
    *,
    ...
    """Copy rows dated on or before 31 December 2025; retain existing lower bounds.

    When available, both year and date must satisfy the cutoff. A known future year
    cannot be rescued by an earlier date (or vice versa). Missing/unparseable dates
    ...
    if not has_year and not has_date:
        raise ValueError(
            "Cannot enforce the analysis cutoff (2025-12-31): "
